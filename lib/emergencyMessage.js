...
function formatLatLng(value) {
  ...
  // Honour the "Precise location" privacy preference: 6 dp (~0.1 m) when precise,
  // 3 dp (~110 m) when the user prefers to share only an approximate area.
  const precise = getPreferences()?.preciseLocation !== false;
  return n.toFixed(precise ? 6 : 3);
}
