...
export function AuthProvider({ children }) {
  ...
  const signIn = useCallback(async ({ email, password }) => {
    ...
    mockStorage.profiles[user.id] = isGovernment
      ? { id: user.id, email: user.email, full_name: 'SafeHer Government Officer', account_type: 'government', agency: 'SafeHer Official' }
      : { id: user.id, email: user.email, full_name: 'SafeHer Citizen', account_type: 'citizen', agency: '' };
    setSession(mockSession);
